...
const isLargeScreen = ref(window.innerWidth > 500)
...
const handleResize = () => {
  isLargeScreen.value = window.innerWidth > 500
}
...
<template>
  <div
    class="w-full h-screen flex flex-col md:flex-row md:p-8"
  ...
  >
    ...
    <div class="w-full flex flex-col md:pl-32">
      ...
      <div class="flex flex-grow flex-col-reverse lg:flex-row gap-8 pt-5 px-6 md:p-0 max-h-[79.5%]">
        <div
          v-if="entries.length"
          class="entries lg:w-1/3 bg-gradientGrayDown shadow-2xl rounded-xl p-5 overflow-auto"
        >
          <h2 class="text-gradient text-3xl font-bold w-fit pb-3">Your Dream Entries</h2>
          <CatNapDreamEntries
            :dreams="entries"
            :date="formattedDate"
            @dreamSelected="handleDreamSelected"
        ...
        >
          ...
          <CatNapReadEntry
            v-if="selectedDream"
            :dream="{ ...selectedDream.dream, date: selectedDream.date }"
